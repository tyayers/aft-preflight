...
export class RESTAIMessagesProxy {
  constructor() {
    DataManager.initializeSync();
  }
  ...
  async handle(req: Request): Promise<Response> {
    ...
    const context = new ApigeeContext(req, {}, "REST-AI-Messages");
    ...
    try {
      ...
      const path = Http.getPath(req.url, "/v1/messages");
      const routes = [
        {
...
export const REST_AI_MessagesInstance = new RESTAIMessagesProxy();

export async function REST_AI_MessagesProxy(req: Request): Promise<Response> {
  return REST_AI_MessagesInstance.handle(req);
}
