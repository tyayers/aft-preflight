...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetchTemplatesAndPopulate().then(() => {
    if (currentTemplateId) {
      loadTemplate(currentTemplateId, true);
    }
  });
...
function initMonaco() {
  ...
  require(["vs/editor/editor.main"], function () {
    ...
    monaco.languages.typescript.javascriptDefaults.addExtraLib(
      apigeeLib,
      "file:///node_modules/@types/apigee/index.d.ts",
    );
...
if (window.location.hash) {
  currentTemplateId = window.location.hash.substring(1);
}

function updateUrlHash(replace = false) {
  const newHash = currentTemplateId ? "#" + currentTemplateId : "";
...
async function loadTemplate(key, replace = false) {
  if (!key) return;
  try {
    currentTemplateId = key;
    updateUrlHash(replace);
...
window.addEventListener("hashchange", () => {
  const templateId = window.location.hash ? window.location.hash.substring(1) : "";
  if (templateId) {
...
async function deleteActiveTemplate() {
  ...
  if (
    !confirm(
      `Are you sure you want to delete the template "${currentTemplateId}"? This action cannot be undone.`,
    )
  ) {
    return;
  }
  ...
  try {
    ...
    await fetchTemplatesAndPopulate();
    if (currentTemplateId) {
      await loadTemplate(currentTemplateId, true);
    } else {
      updateUrlHash(true);
      processYamlInput("", true);
      syncMonacoValues();
...
async function triggerServerRebuild(options = {}) {
  const { silent = false, reload = false, button = null } = options;
  const origText = button ? button.innerHTML : "";

  if (button) {
    button.disabled = true;
    button.innerHTML = `<i data-lucide="loader" class="spin"></i> Rebuilding...`;
  ...
  try {
    ...
    if (!silent) {
      if (reload) {
        ...
        showToast("Rebuild successful!", "success");
      }
    }
...
function processYamlInput(yamlText, forceRebuild = false) {
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      document.getElementById("summary-proxy-name").textContent =
        `Proxy: ${proxyData.name || "Unnamed"}`;
...
function renderVisualCanvas() {
  ...
  if (proxyData.endpoints && proxyData.endpoints.length > 0) {
    proxyData.endpoints.forEach((ep, epIdx) => {
      ...
      if (
        epIdx < proxyData.endpoints.length - 1 ||
        (proxyData.targets && proxyData.targets.length > 0)
      ) {
        canvas.appendChild(createConnectorArrow());
      }
    });
...
function renderFlowsContainer(flows, parentType, parentIdx) {
  ...
  return flows
    .map((fl, flIdx) => {
      ...
      const modeBadge = fl.mode
        ? `<span class="flow-mode-badge ${fl.mode.toLowerCase()}">${fl.mode}</span>`
        : "";
...
function renderInspector() {
  ...
  if (selectedItem.type === "proxy") {
    ...
    const flowsList =
      selectedItem.parentType === "endpoint"
    ...
    if (policyObj.type === "Javascript") {
      ...
      if (!resourceObj && filename) {
        ...
        resourceObj = {
          name: filename,
          type: "jsc",
          content: "// Write your javascript script here...\n",
        };
...
function updateJsResourceContent(jsContent) {
  if (selectedItem && selectedItem.type === "step") {
    ...
    if (
      policyObj &&
      policyObj.type === "Javascript" &&
      policyObj.content &&
      policyObj.content.javascript
    ) {
...
function handleStepAction(action, parentType, parentIdx, flowIdx, stepIdx) {
  const epOrTgt =
    parentType === "endpoint" ? proxyData.endpoints[parentIdx] : proxyData.targets[parentIdx];
...
function setupDragAndDrop() {
  ...
  canvas.addEventListener("dragover", (e) => {
    ...
    if (
      stepNode.dataset.parentType === draggedStepElement.dataset.parentType &&
