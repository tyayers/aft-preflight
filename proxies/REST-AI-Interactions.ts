...
export class RESTAIInteractionsProxy {
  constructor() {
    DataManager.initializeSync();
  }
  ...
  async JS_CheckModel(context: ApigeeContext, request: ApigeeRequest, response: ApigeeResponse): Promise<void> {
    ...
    if (!modelMapping || modelMapping === "gemini-flash-latest=gemini-3.7-flash,gemini-2.5-flash=gemini-3.7-flash" || modelMapping.indexOf("=") === -1) {
    modelMapping = "gemini-flash-latest=gemini-3.7-flash,gemini-2.5-flash=gemini-3.7-flash";
    ...
    if (!modelRouting || modelRouting === "'google/*'=gemini" || modelRouting.indexOf("=") === -1) {
    modelRouting = "'google/*'=gemini";
  ...
  async handle(req: Request): Promise<Response> {
    ...
    const context = new ApigeeContext(req, {}, "REST-AI-Interactions");
    ...
    try {
      ...
      context.setVariable("propertyset.ai.ModelRouting", "'google/*'=gemini");
      ...
      if (context.getVariable("propertyset.ai.ModelRouting") === undefined) { context.setVariable("propertyset.ai.ModelRouting", "'google/*'=gemini"); }
      ...
      const path = Http.getPath(req.url, "/v1beta/interactions");
      const routes = [
        {
          "name": "route-gemini",
          "condition": "target.route = \"gemini\"",
          "target": "gemini"
        },
        {
          "name": "default"
      ...
      if (!targetObj && (selectedTargetName === "default" || !targetsMap[selectedTargetName])) {
        targetObj = targetsMap["gemini"] || Object.values(targetsMap)[0];
      }
      const rawTargetUrl = targetObj?.url || "https://generativelanguage.googleapis.com/v1beta/interactions";
...
export const REST_AI_InteractionsInstance = new RESTAIInteractionsProxy();

export async function REST_AI_InteractionsProxy(req: Request): Promise<Response> {
  return REST_AI_InteractionsInstance.handle(req);
}
