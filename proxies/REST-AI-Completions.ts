...
export class RESTAICompletionsProxy {
  ...
  async handle(req: Request): Promise<Response> {
    ...
    try {
      ...
      await context.traceStep("OAS-Validation", "OASValidation", "Request Flow", async () => {
        await Apigee.oasValidation({
          "policyName": "OAS-Validation",
          "oasResource": "oas://openapi.yaml",
          "validateMessageBody": true,
      ...
      // Execute FaultRules
        if (Apigee.evaluateCondition("fault.name == \"InvalidApiKey\" OR fault.name == \"FailedToResolveAPIKey\"", context)) {
          await context.traceStep("AM-Unauthorized", "AssignMessage", "FaultRules", async () => {
          await Apigee.assignMessage({
            "assignTo": "request",
            "ignoreUnresolvedVariables": true,
            "setPayload": "Unauthorized.",
            "setStatusCode": 401
          }, context);
        });
        } else {
